...
Reads (from repo, read-only):
  ...
  <repo>/index.csv           (optional, Food Diary meal log)
...
def build_recommendations(daily, workouts, meals, today, year_ago):
    ...
    if meals is None or len(meals) == 0:
        nutrition.append({
            ...
            "action": "Send food/receipt photos directly to Food Diary. Need ≥14 days of "
        ...
        if days_logged < 14:
            nutrition.append({
                ...
                "action": (f"Need ≥14 consecutive days of logging for any nutrition "
                           f"signal. Currently at {days_logged}/14 — keep using Food Diary. "
...
def main():
    ...
    p = doc.add_paragraph()
    run = p.add_run("Generated by the food-diary skill from Apple Health + Food Diary "
                    "meal log. Source data lives in the health-log git repo and is "
    ...
    # Meal log
    doc.add_heading("Meal log (Food Diary)", level=1)
    if meals is None or len(meals) == 0:
        p = doc.add_paragraph()
        r = p.add_run("No meal entries yet. Send food photos directly to Food Diary to start logging.")
        r.italic = True
    else:
        n_meals = len(meals)
    ...
    r = p.add_run("This report is regenerable from the source CSVs in "
                  "~/Desktop/health-log/health/ plus the Food Diary meal log. "
    ...
    if len(sys.argv) == 3:
        ...
        out_path = os.path.expanduser(f"~/Desktop/food-diary-health-report-{today}.docx")
    doc.save(out_path)
